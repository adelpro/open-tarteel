...
const makeFetchResponse = (data: unknown): Promise<Response> =>
  Promise.resolve({
    ok: true,
...
const makeEditionsResponse = (): QuranAiEditionListResponse => ({
  code: 200,
  status: 'OK',
  data: [
    makeEdition({}),
...
const PROBE_IBRAHIM_URL =
  'https://quranhub.b-cdn.net/quran/audio/surah/48/ar.ibrahimakhdar.hafs/1.mp3';

const defaultFetchMock = vi.fn().mockImplementation((url: string) => {
